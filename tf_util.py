...
def chamfer_signle(pcd1,pcd2):
    dist1, _, dist2, _ = tf_nndistance.nn_distance(pcd1, pcd2)
    dist1 = tf.reduce_mean(tf.sqrt(dist1))
    return dist1

def chamfer_samesize(A,B):
...
def get_edge_feature(point_cloud, nn_idx, k=20):
    ...
    print("point cloud shape: ",point_cloud_central.shape)
    ...
    print("idx shape: ",idx_.shape)
    idx_ = tf.reshape(idx_, [batch_size, 1, 1])
    print("idx shape: ",idx_.shape)

    point_cloud_flat = tf.reshape(point_cloud, [-1, num_dims])
    print("point_cloud_flat.shape ",point_cloud_flat.shape)
    point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx + idx_)
    print((nn_idx + idx_).shape)
    print("point cloud neighbors: ",point_cloud_neighbors.shape)
    point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)
    print("point_cloud_central.shape ", point_cloud_central.shape)
