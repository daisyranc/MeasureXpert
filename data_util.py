...
def ordersample_pcd(pcd, n):
    """Only select the front 16384 points as the ground truth"""
    return pcd[0:n]
...
class PreprocessData(dataflow.ProxyDataFlow):
    ...
    def get_data(self):
        for (id, partial_front,head_front,right_arm_front,left_arm_front,right_leg_front,left_leg_front,body_front,posed_front,
             partial_back,head_back,right_arm_back,left_arm_back,right_leg_back,left_leg_back,body_back,posed_back,t_landmark,t_front,t_back,values) in self.ds.get_data():
            # gt = input
            partial_front=resample_pcd(partial_front, self.input_size)
            partial_back=resample_pcd(partial_back, self.input_size)
            ...
            body_back=body_back
            posed_back=posed_back
            t_landmark=t_landmark
            t_front=t_front
            t_back=t_back
            values=values
            yield id, partial_front,head_front,right_arm_front,left_arm_front,right_leg_front,left_leg_front,body_front,posed_front,\
                  partial_back,head_back,right_arm_back,left_arm_back,right_leg_back,left_leg_back,body_back,posed_back,t_landmark,t_front,t_back,values
...
class PreprocessData_offset(dataflow.ProxyDataFlow):
    def __init__(self, ds, input_size, output_size):
        super(PreprocessData_offset, self).__init__(ds)
        self.input_size = input_size
        self.output_size = output_size
#pred_key_landmark,pred_t,key_landmarks,gt,all_values
    def get_data(self):
        for (id, pred_key_landmark,pred_t,key_landmarks,gt,all_values) in self.ds.get_data():
            pred_landmark_t=np.vstack((pred_key_landmark, pred_t))
            gt_all=np.vstack((key_landmarks, gt))
            yield id, pred_landmark_t,gt_all,all_values

def lmdb_dataflow_offset(lmdb_path, batch_size, input_size,output_size, is_training, test_speed=False):
    ...
    df = PreprocessData_offset(df, input_size, output_size)
    if is_training:
